...
Reviewed implementation of production safety defaults hardening including Postgres preset config, plugin metadata version fix, and worker event-based wakeup.

### AC Verification

| Criterion | Evidence |
|-----------|----------|
| AC1: Postgres warning | `docs/plugins/sinks/postgres.md:46-60` - Warning block with DDL caution |
| AC2: Production preset postgres | `src/fapilog/core/presets.py:43-45` - `create_table: False` |
| AC3: Preset docs updated | `docs/user-guide/configuration.md:120` - Postgres line added |
| AC4: Metadata version | `src/fapilog/plugins/metadata.py:204` - Changed to `"0.1.0"` |
| AC5: Worker parameter | `src/fapilog/core/worker.py:159` - `enqueue_event` parameter |
| AC6: Immediate wakeup | `src/fapilog/core/worker.py:229-248` - Event wait logic |
