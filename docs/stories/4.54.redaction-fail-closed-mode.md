...
# Metrics via record_redaction_exception() should be called
```
...
# 3. Call metrics.record_redaction_exception()
```
...
src/fapilog/core/worker.py (MODIFIED - honor fail mode in _apply_redactors, update _flush_batch)
src/fapilog/core/presets.py (MODIFIED - set warn mode for production presets)
src/fapilog/metrics/metrics.py (MODIFIED - add record_redaction_exception method)
...
# metrics.py - add to MetricsCollector
...
# Also add in __init__ (lazy initialization section):
self._c_redaction_exceptions: Any | None = None
# And in the if self._enabled block:
...
# worker.py - add to LoggerWorker.__init__ parameters
def __init__(
    self,
    *,
    # ... existing parameters ...
    redaction_fail_mode: Literal["open", "closed", "warn"] = "open",  # NEW
    # ... rest of parameters ...
) -> None:
    # ... existing assignments ...
    self._redaction_fail_mode = redaction_fail_mode  # NEW

# worker.py - updated _apply_redactors (return type changes to Optional)
...
# worker.py - update _flush_batch to handle None from _apply_redactors
# In the batch processing loop (around line 272), change:
...
# fallback.py - add _extract_bytes helper
...
# presets.py - add redaction_fail_mode to production/fastapi/serverless presets
# Note: Base default in CoreSettings is "open". Presets override to "warn".

# In PRESETS["production"]["core"], add:
"redaction_fail_mode": "warn",

# In PRESETS["fastapi"]["core"], add:
"redaction_fail_mode": "warn",

# In PRESETS["serverless"]["core"], add:
"redaction_fail_mode": "warn",

# Example of full production preset after change:
"production": {
    "core": {
        "log_level": "INFO",
        "batch_max_size": 100,
        "drop_on_full": False,
        "redaction_fail_mode": "warn",  # NEW - emit warning on redactor exceptions
...
- [ ] Add `record_redaction_exception()` method to `MetricsCollector`
- [ ] Add `_redaction_fail_mode` parameter to `LoggerWorker.__init__`
- [ ] Update `LoggerWorker._apply_redactors()` to honor fail mode and return `None` for closed mode
- [ ] Update `_flush_batch()` to handle `None` return from `_apply_redactors()` (drop event, increment counters)
...
Reviewed implementation of redaction fail-closed mode and fallback hardening. All 6 acceptance criteria verified with specific code evidence.

### AC Verification

| Criterion | Evidence |
|-----------|----------|
| AC1: Global redaction fail mode setting | `settings.py:790-798` - Literal field; `presets.py:31,63,90` - preset defaults |
| AC2: Fail-closed mode drops events | `worker.py:386-396` - returns None; `worker.py:274-280` - handles None |
| AC3: Warn mode logs diagnostic | `worker.py:398-407` - emits warning, returns original |
| AC4: Serialized fallback applies redaction | `fallback.py:128-140` - deserialize/redact/reserialize |
| AC5: Invalid JSON handled gracefully | `fallback.py:141-148` - catches errors, falls back to raw |
| AC6: Documentation updated | `behavior.md:151-204` - comprehensive setting explanations |
...
| 2026-01-27 | Review fixes: corrected metrics API (record_redaction_exception), added __init__ injection, added _flush_batch None handling, added _extract_bytes helper, added explicit preset configuration examples | Claude |
| 2026-01-27 | Code review passed, status updated to Complete | Claude |
