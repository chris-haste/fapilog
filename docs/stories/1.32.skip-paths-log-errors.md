...
The `skip_paths` feature in `LoggingMiddleware` allows users to exclude high-frequency endpoints (e.g., `/health`, `/metrics`) from logging to reduce noise. However, the current implementation performs a hard skip—errors on skipped paths are completely silenced.
