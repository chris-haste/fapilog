...
Currently, integrating fapilog with FastAPI requires ~30-40 lines of boilerplate code for a complete setup with lifespan management, middleware, and logger dependencies:

**Current experience** (best practice with full setup):

```python
# 1. Manual lifespan manager (~18 lines)
...
# 2. Manual middleware addition (~4 lines)
...
# 3. Manual logger dependency (~5 lines)
# Note: Context vars (request_id, user_id, etc.) are automatically included
# via the context_vars enricher when using the fastapi preset or enabling it
...
**Total: ~30-40 lines of boilerplate**

**Note**: The `context_vars` enricher (enabled in the `fastapi` preset) automatically includes `request_id`, `user_id`, `tenant_id`, `trace_id`, and `span_id` from context variables set by `RequestContextMiddleware`. Manual binding of these fields is unnecessary.
...
**Total: 1 line of setup, automatic context propagation**

This story implements the `setup_logging()` helper and `get_request_logger` dependency to reduce FastAPI integration from ~30-40 lines to 1 line (lifespan parameter), representing an ~80% reduction in setup code.
...
def setup_logging(
    ...
    *,
    ...
    - Adds middleware automatically by default
    ...
    Args:
        ...
        auto_middleware: Automatically register RequestContext/Logging middleware
...
# Example 4: Attach lifespan after app creation
app = FastAPI()
app.router.lifespan_context = setup_logging(preset="production")

# Example 5: Disable auto middleware for manual control
app = FastAPI(lifespan=setup_logging(preset="dev", auto_middleware=False))
...
- Automatically adds middleware in correct order by default
...
async def get_request_logger(request: Request | None = None) -> AsyncLoggerFacade:
    ...
    Usage:
        ...
        async def get_user(
            ...
            #   "user_id": 123,
            #   "request_id": "abc-123",  # Auto-added via context_vars enricher
            # }
            # Note: method, path, status_code, latency_ms are logged separately
            # by LoggingMiddleware in the request_completed event
...
- Context (request_id, user_id, trace_id, span_id) automatically included via `context_vars` enricher
- No manual binding needed (RequestContextMiddleware sets context variables, enricher includes them)
- Can be used in routes, dependencies, background tasks
- Thread-safe and async-safe

**Auto-included context fields** (via `context_vars` enricher, set by RequestContextMiddleware):
...
**Note**: The `context_vars` enricher must be enabled (it's included in the `fastapi` preset). If using a custom preset, ensure `context_vars` is in the enrichers list.

### AC3: Automatic Middleware Registration

**Automatic middleware registration** (default):
...
**Disable auto middleware** for manual control:
...
**Before (current best practice):**

```python
# ~30-40 lines of boilerplate

# 1. Lifespan manager (~18 lines)
from contextlib import asynccontextmanager
from fapilog import get_async_logger
...
# 3. Logger dependency (~5 lines)
# Context vars (request_id, user_id, etc.) are auto-included via enricher
async def get_logger():
    return await get_async_logger("request")
...
**Line reduction: ~30-40 lines → 1 line setup (~80% reduction)**

**Key improvements:**
- Automatic lifespan management (logger drain on shutdown)
- Automatic middleware registration in correct order
- Built-in `get_request_logger` dependency
- Preset integration for easy configuration
...
**Decision: Auto-configure middleware by default**

```python
# Default: auto middleware
app = FastAPI(lifespan=setup_logging(preset="production"))

# Manual control: disable auto middleware
app = FastAPI(lifespan=setup_logging(auto_middleware=False))
app.add_middleware(RequestContextMiddleware)
app.add_middleware(LoggingMiddleware)
```
...
- Best DX: one-liner includes middleware by default
- Flexible: users can opt out for custom ordering
...
async def get_request_logger(request: Request | None = None):
    ...
    # Prefer the logger created by setup_logging, fall back to default
    if request is not None:
        try:
            return request.app.state.fapilog_logger
        except Exception:
            pass
...
async def setup_logging(
    ...
    *,
    ...
    # Configure middleware by default
    if auto_middleware:
...
def _configure_middleware(
    ...
    *,
    ...
    # Order matters: FastAPI prepends middleware, so add Logging first.
    app.add_middleware(
        LoggingMiddleware,
        logger=logger,
        skip_paths=skip_paths,
...
class TestMiddlewareConfiguration:
    ...
    def test_middleware_added_by_default(self):
        """Middleware automatically added by default."""
        app = FastAPI(lifespan=setup_logging(preset="production"))
...
class TestBeforeAfterComparison:
    ...
    def test_before_requires_30_plus_lines(self):
        """Before Story 10.3: ~30-40 lines of boilerplate."""
        # This test documents the old way (for comparison)

        # 1. Manual lifespan (~18 lines)
        app_logger = None
        ...
        # 3. Manual logger dependency (~5 lines)
        # Note: Context vars are auto-included via enricher, no manual binding needed
        async def get_logger_dep():
            from fapilog import get_async_logger
            return await get_async_logger("request")
        ...
        async def root(logger=Depends(get_logger_dep)):
            await logger.info("Request")  # request_id auto-included
            return {"status": "ok"}

        # Total: ~30-40 lines

    def test_after_requires_1_line(self):
        """After Story 10.3: 1 line of setup."""
        from fapilog.fastapi import get_request_logger, setup_logging
        ...
        # Total: 1 line setup + routes (~80% reduction)
```
...
class TestConfigureMiddleware:
    ...
    def test_adds_middleware_in_correct_order(self):
        ...
        # Verify LoggingMiddleware added first (order reversed by FastAPI)
        first_call = app.add_middleware.call_args_list[0]
        assert "LoggingMiddleware" in str(first_call)
    ...
    def test_passes_config_to_logging_middleware(self):
        ...
        logging_call = calls[0]  # First call
...
- [ ] Test: Middleware auto-configured by default
...
- [ ] Line reduction: ~30-40 → 1 line (~80% reduction)
