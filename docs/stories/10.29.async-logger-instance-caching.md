...
Implements logger instance caching with `reuse` parameter, `ResourceWarning` on undrained GC, and cache management functions. All core ACs verified with tests.

### AC Verification

| Criterion | Evidence |
|-----------|----------|
| AC1: Same Name Returns Same Instance | `src/fapilog/__init__.py:693-694` cache check; `tests/unit/test_logger_caching.py:18-25` |
| AC2: Reuse Parameter Allows Opt-Out | `src/fapilog/__init__.py:700` param; `tests/unit/test_logger_caching.py:47-56` |
| AC3: ResourceWarning on Undrained GC | `src/fapilog/core/logger.py:574-603`; `tests/unit/test_logger_resource_warning.py:19-56` |
| AC4: Cache Management Functions | `src/fapilog/__init__.py:938-983`; `tests/unit/test_logger_caching.py:111-148` |
| AC5: Thread-Safe Cache Access | `src/fapilog/__init__.py:69,744-756`; `tests/unit/test_logger_caching.py:156-168` |
| AC6: Documentation Updated | Docstrings updated; external docs deferred to follow-up |
| AC7: runtime_async() Independent | `src/fapilog/__init__.py:863`; `tests/unit/test_logger_caching.py:175-199` |
