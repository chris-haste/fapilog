...
- **10 tests** marked `@pytest.mark.flaky` due to timing issues
- **Hard sleeps** (1.0s, 1.2s) that assume consistent CI runner performance
- **28+ uses** of blocking `time.sleep()` in tests
...
| 11.1 | [Replace Hard Sleeps with Event-Based Coordination](./11.1.replace-hard-sleeps-event-coordination.md) | Critical | 1 day | Planned | None |
...
| 11.5 | [Replace Blocking time.sleep in Async Contexts](./11.5.replace-blocking-sleep-async.md) | Medium | 1 day | Planned | None |
...
### Phase 1: Critical Fixes (11.1 - 11.2)

**Goal:** Eliminate the most impactful sources of CI flakiness

| Story | Title | Description | Status |
|-------|-------|-------------|--------|
| 11.1 | Replace Hard Sleeps | Replace 1.0s/1.2s sleeps in integration tests with event coordination | Planned |
| 11.2 | Fix Flaky Tests | Address root causes of 10 `@pytest.mark.flaky` tests | Planned |

**Success Criteria:**
- [ ] Zero tests with hard sleeps > 0.5s
- [ ] All 10 flaky tests either fixed or removed

### Phase 2: Standardization (11.3 - 11.5)
...
| 11.5 | Async Sleep Migration | Replace blocking `time.sleep()` with `await asyncio.sleep()` | Planned |

**Success Criteria:**
- [ ] All Hypothesis tests respect `HYPOTHESIS_MAX_EXAMPLES` env var
- [ ] CI can scale timeouts via environment variable
- [ ] No blocking sleeps in async test contexts

### Phase 3: Performance Optimization (11.6 - 11.9)
...
|--------|-------|
| Flaky tests | 10 |
| Hard sleeps > 0.5s | 2 (1.0s, 1.2s) |
| `time.sleep()` uses in tests | 28+ |
...
| Flaky tests | 0 |
| Hard sleeps > 0.5s | 0 |
| Blocking sleeps in async context | 0 |
| Test files > 500 lines | 0 |
...
- [ ] All integration test sleeps use event-based coordination
...
|------|--------|------------|
| Event-based refactoring introduces new races | Medium | Add explicit assertions for event ordering |
