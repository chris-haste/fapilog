...
Simplified `serialize_envelope()` to trust upstream v1.1 schema compliance. Removed strict validation for context/diagnostics (now provides defaults). Updated sink/worker comments and diagnostics to indicate truly exceptional error path.

### AC Verification

| Criterion | Evidence |
|-----------|----------|
| AC1: serialize_envelope() Trusts Input Schema | `serialization.py:237-239` - minimal validation; `test_serialization_cleanup.py:35-47` - contract test |
| AC2: Sinks Use Direct Serialization | `stdout_json.py:46-66`, `rotating_file.py:140-158` - updated comments |
| AC3: Worker _try_serialize() Simplified | `worker.py:421-464` - docstring and comments updated |
| AC4: strict_envelope_mode Works Correctly | `test_worker_lifecycle.py:442-505` - validates strict mode |
| AC5: Serialization Failure Metrics | `worker.py:448-456` - diagnostics.warn() for failures |
| AC6: Test Updates for Schema Alignment | `test_serialization_recovery.py:15-30`, `test_worker_lifecycle.py:449-453` |
| AC7: Performance Validation | Implicit - exception removal verified by tests |
| AC8: Documentation Updated | `serialization.py:1-18`, `CHANGELOG.md` |
...
- [x] No dead code (vulture false positives only)
