...
- Builder is immutable (each method returns new builder or modifies internal state)
...
class LoggerBuilder:
    ...
    def build(self) -> Any:
        ...
        if self._preset:
            ...
            preset_config = get_preset(self._preset)
            # Merge preset config with builder config
            self._merge_config(preset_config, self._config)

        # Add sinks to config
        if self._sinks:
            sink_names = [s["name"] for s in self._sinks]
            self._config.setdefault("core", {})["sinks"] = sink_names
            ...
            if sink_config:
                self._config["sink_config"] = sink_config
