...
class UniversalSettings(BaseModel):
    ...
    level: LogLevel = Field(
        default=LogLevel.INFO, description="Logging level for the system"
    )
    sinks: List[str] = Field(
        default=["stdout"], description="List of sink URIs to write logs to"
    )

    # Async processing settings
    async_processing: bool = Field(
        default=True, description="Enable async-first processing"
    )
    ...
    zero_copy_operations: bool = Field(
        default=True, description="Enable zero-copy operations for maximum performance"
    )
    parallel_processing: bool = Field(
        default=True, description="Enable parallel processing of events"
    )
    ...
    queue_max_size: int = Field(
        default=10000, ge=100, le=1000000, description="Maximum size of the async queue"
    )
    ...
    data_minimization: bool = Field(
        default=False, description="Enable automatic PII detection and redaction"
    )
    audit_trail: bool = Field(
        default=False, description="Enable immutable audit trail logging"
    )
    encryption_enabled: bool = Field(
        default=False, description="Enable log encryption for sensitive data"
    )

    # Plugin ecosystem settings
    plugins_enabled: bool = Field(
        default=True, description="Enable the universal plugin ecosystem"
    )
    plugin_marketplace: bool = Field(
        default=True, description="Enable plugin marketplace functionality"
    )
    plugin_auto_discovery: bool = Field(
        default=True, description="Enable automatic plugin discovery"
    )
    ...
    alerting_enabled: bool = Field(
        default=False, description="Enable future alerting capabilities"
    )
    alerting_plugins: List[str] = Field(
        default=[], description="List of alerting plugin URIs"
    )

    # Event structure settings
    include_source: bool = Field(
        default=True, description="Include source field in log events"
    )
    include_severity: bool = Field(
        default=True, description="Include severity field in log events"
    )
    include_tags: bool = Field(
        default=True, description="Include tags field in log events"
    )
    include_metrics: bool = Field(
        default=True, description="Include metrics field in log events"
    )
    include_correlation: bool = Field(
        default=True, description="Include correlation_id field in log events"
    )

    # Custom configuration
    custom_settings: Dict[str, Any] = Field(
        default_factory=dict, description="Custom settings for plugins and extensions"
    )

    @field_validator("sinks")  # type: ignore[misc]
    ...
    @field_validator("custom_settings")  # type: ignore[misc]
    def validate_custom_settings(cls, v: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in v.keys():
            if key in reserved_keys:
                raise ValueError(f"Custom setting '{key}' conflicts with reserved key")
