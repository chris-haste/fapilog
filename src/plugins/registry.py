...
class PluginRegistry:
    ...
    async def register_processor_plugin(self, name: str, plugin: AsyncProcessorPlugin) -> None:
        """Register a processor plugin."""
        async with self._lock:
            self._processor_plugins[name] = plugin
    
    async def register_enricher_plugin(self, name: str, plugin: AsyncEnricherPlugin) -> None:
    ...
    async def cleanup(self) -> None:
        """Cleanup all plugins."""
        async with self._lock:
            for plugin in self._sink_plugins.values():
                await plugin.cleanup()
            for plugin in self._processor_plugins.values():
                await plugin.cleanup()
            for plugin in self._enricher_plugins.values():
                await plugin.cleanup()
